...
static int ** allocate_array(int N)
{
    ...
    #pragma omp parallel for default(none) firstprivate(N,array)
    for (i = 0; i < N ; i++)
        array[i] = malloc(N * sizeof(int));
    #pragma omp parallel for collapse(2) default(none) firstprivate(N,array)
    for (i = 0; i < N ; i++)
        for (j = 0; j < N ; j++)
            array[i][j] = 0;
    return array;
...
{
    int i, pos;
    #pragma omp parallel for shared(array1,array2) private(i,pos) firstprivate(N) default(none)
    for (i = 0 ; i < (N * N)/10 ; i++)
    {
        pos = rand() % ((N-2)*(N-2));
...
static void print_to_pgm(int ** array, int N, int t)
{
    ...
    FILE * f = fopen(s, "wb");
    fprintf(f, "P5\n%d %d 1\n", N,N);
    #pragma omp parallel for shared(array) collapse(2) firstprivate(N,f,s) default(none)
    for (i = 0; i < N ; i++)
        for (j = 0; j < N ; j++)
            if (array[i][j] == 1)
                fputc(1, f);
...
int main (int argc, char * argv[])
{
    ...
    #pragma omp parallel for firstprivate(N,T,nbrs) collapse(3) shared(previous,current) lastprivate(swap)
    for (t = 0 ; t < T ; t++)
    {
        for (i = 1 ; i < N-1 ; i++)
            for (j = 1 ; j < N-1 ; j++)
            {
                ...
                #pragma omp critical					//Swap current array with previous array
