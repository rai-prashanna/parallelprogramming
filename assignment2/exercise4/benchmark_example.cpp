...
#include "include/sorted_list.hpp"
...
void mixed(List& l, int random) {
	...
	if(choice == 0) {
	std::cout<<"\n inside insert-mixed operation";
		l.insert(random % DATA_VALUE_RANGE_MAX);
	} else if(choice == 1) {
		std::cout<<"\n inside remove-mixed operation";
		l.remove(random % DATA_VALUE_RANGE_MAX);
	} else {
		std::cout<<"\n inside count-mixed operation";
